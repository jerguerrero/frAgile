...
const App = (props) => {
  function changeBackground(color) {
    document.body.style.background =
      'url(https://media1.giphy.com/media/aRZ4vTsHnyW6A/giphy.gif)'
    document.body.style.fontFamily = `"COMIC SANS", "COMIC RELIEF", PAPYRUS, cursive`
    document.body.style.color = 'red'
    var x = document.getElementsByClassName(
      'MuiTypography-root MuiListItemText-secondary MuiTypography-body2 MuiTypography-colorTextSecondary'
    )
    ...
    // element that will be wrapped
    var el3 = document.querySelectorAll(
    ...
    var el4 = document.getElementsByClassName(
      'MuiPaper-root MuiPaper-elevation1 MuiCard-root MuiPaper-rounded'
    )

    for (var l = 0; l < el4.length; l++) {
      el4[l].style.background =
        'url(https://d2gg9evh47fn9z.cloudfront.net/800px_COLOURBOX26992147.jpg)'
    }
    ...
    var el6 = document.getElementsByClassName(
    ...
    el7.style.backgroundImage =
      'linear-gradient(to left, violet, indigo, blue, green, yellow, orange, red)'

    var el8 = document.getElementById('middlepane')
    el8.style.background =
      'url(https://media.giphy.com/media/UgxKMnMAaS4co/giphy.gif)'
  }
      ...
      return (
        <>
          <MenuItem onClick={handleClose}>
            <Link to="/Invite" className={'button'}>
              Invite User
            </Link>
          </MenuItem>
          <MenuItem onClick={handleClose}>
            <Link to="/ArtifactManagement" className={'button'}>
              Pass Down Artifact
            </Link>
          </MenuItem>
          <MenuItem onClick={handleClose}>
      ...
      <div>
        <AppBar id={'mainappbar'} position="static">
          <Grid
            ...
            spacing={2}
          >
            <IconButton onClick={() => changeBackground('red')}></IconButton>
            <Grid item>
              <Link to="/" className={'button'}>
                <h1 id={'titleOfApp'} style={{ color: 'white' }}>
                  {'Heirloom'}
                </h1>
              </Link>
            </Grid>
            ...
            <Grid item style={{ position: 'absolute', right: '30px' }}>
              <Button
                aria-controls="admin-bar"
                aria-haspopup="true"
                onClick={handleClick}
              >
                <h4 style={{ color: 'white' }}>
                  {'Hi '}
                  {get(user, 'displayName', 'user')}
                </h4>
              </Button>
              <Menu
                id="admin-bar"
